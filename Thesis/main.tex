...
Also, I would like to thank my wife, Shahrzad Pourjafargholi to support me in this journey.
...
\section{Repository Metadata and Metrics}

After library detection, the study enriches the dataset with GitHub repository metadata for detected open-source libraries. These metadata are later used to construct three repository-level metrics: popularity, activity, and fragility. These metrics provide additional context beyond simple library presence and support the later analysis of which APKs depend on libraries with higher fragility.

\section{Methodological Limitations}
The method has several limitations. The APK corpus is limited by hardware resources. The pipeline is designed to work on local device. As a result hardware limitation is inevitable. The corpus is not category-balanced. The analysis is static, so it detects embedded code evidence but does not prove runtime execution. The method also assume on successful APK decoding, partial available license evidence, and availability of public repository URLs; missing or obfuscated information can reduce detection coverage.
...
\section{GitHub-Based Open Source Library Metrics}
After analyzing APK-level metrics, we now move to repository-level evaluation using GitHub metadata. In this section, we will see how three composite metrics are constructed: popularity, activity, and fragility. These metrics are computed before integrating GitHub data with the APK dataset, so the focus here is on how they are calculated rather than on their final values.

Previous studies show that GitHub metadata such as stars, forks, watchers/subscribers, commits, update recency, issues, repository age, and licensing information are frequently used to evaluate repository visibility, maintenance, adoption suitability, and project health.

\subsection{Library Popularity}
Library popularity measures how widely a repository appears both in the APK corpus and on GitHub. The metric combines corpus-derived usage indicators with GitHub visibility indicators. The APK-level indicators include \texttt{library\_occurrences} and \texttt{apk\_count}, while GitHub indicators include \texttt{stars\_count}, \texttt{forks\_count}, and \texttt{subscribers\_count}.

This design follows prior work that treats GitHub stars, forks, and watchers/subscribers as signals of repository popularity and social attention. \textcite{borgesUnderstandingFactorsThat2016} describe GitHub stars as an explicit way for users to express interest or satisfaction with a repository and study stars as a proxy for repository popularity. In addition, open source component-selection research shows that practitioners consider popularity, community adoption, and repository metadata when deciding whether to integrate an open-source component \parencite{liExploringFactorsMetrics2022}. Based on this literature, the popularity score in this thesis combines GitHub visibility with direct evidence of how often a library appears in the analyzed APK corpus.

\begin{equation}
\begin{split}
Popularity = & \,0.30 \times norm(library\_occurrences) \\
& + 0.25 \times norm(apk\_count) \\
& + 0.20 \times norm(log(1 + stars)) \\
& + 0.15 \times norm(log(1 + forks)) \\
& + 0.10 \times norm(log(1 + subscribers))
\end{split}
\end{equation}

Here, \texttt{norm()} scales indicators to a comparable range, while \texttt{log(1 + x)} reduces the effect of very large GitHub counts and keeps zero values valid. The higher weights assigned to \texttt{library\_occurrences} and \texttt{apk\_count} emphasize observed use in the APK corpus, while the GitHub indicators capture broader ecosystem visibility. The resulting popularity ranking for repositories detected in the APK corpus is shown in Figure~\ref{fig:github-popularity-score}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth,keepaspectratio]{image/top_repositories_by_popularity_score.png}
\caption{Top 10 repositories by popularity score in the APK corpus}
\label{fig:github-popularity-score}
\end{figure}

\subsection{Repository Activity Metrics}
Repository activity estimates how actively a project is maintained. The metric is based on temporal and structural indicators, including \texttt{days\_since\_push}, \texttt{days\_since\_update}, \texttt{repo\_age\_days}, and \texttt{forks\_count}.

The activity metric is derived from research on open source library maintenance and project health. \textcite{coelhoThisGitHubProject2020} argue that users need indicators of whether a GitHub project is maintained and propose measuring maintenance activity using repository data such as commits, issues, pull requests, forks, and activity history. Similarly, \textcite{linakerHowCharacterizeHealth2022} define open source library health as the capability of a project to remain viable and maintained over time, and identify technical activity as an important dimension of project health. Therefore, recent push and update activity are treated as the strongest activity signals in this thesis.

Recent activity is treated as a positive signal. Therefore, the metrics for time since last push and update are inversely normalized, meaning that smaller values produce higher scores. Repository age and forks are included with smaller weights as indicators of maturity and community engagement.

\begin{equation}
\begin{split}
Activity = & \,0.45 \times norm^{-1}(days\_since\_push) \\
& + 0.35 \times norm^{-1}(days\_since\_update) \\
& + 0.10 \times norm(log(1 + repo\_age\_days)) \\
& + 0.10 \times norm(log(1 + forks))
\end{split}
\end{equation}

Here, \texttt{norm()\textsuperscript{-1}} reverses the normalized scale, so smaller values such as fewer days since the last push or update produce higher activity scores. The weighting emphasizes maintenance recency because recently pushed or updated repositories are more likely to be actively maintained than repositories with long periods of inactivity. The repository activity ranking for libraries detected in the APK corpus is shown in Figure~\ref{fig:github-activity-score}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth,keepaspectratio]{image/top_repositories_by_activity_score.png}
\caption{Top 10 repositories by activity score in the APK corpus}
\label{fig:github-activity-score}
\end{figure}

\subsection{Fragility Metrics}
Fragility estimates potential maintenance or governance risk in a repository. A higher score indicates that a library may be less stable or more risky as a dependency.

The fragility metric is derived from literature on open source library health, maintenance risk, and component selection. \textcite{coelhoThisGitHubProject2020} explicitly link maintenance activity to the risk of depending on unmaintained GitHub projects. \textcite{liExploringFactorsMetrics2022} also identify factors such as license, community activity, issue management, and project characteristics as relevant when selecting open source library components for integration. In this thesis, fragility is therefore modeled as a risk-oriented repository health indicator.

The metric combines several signals: repository state, maintenance recency, issue volume, popularity, and license availability. Archived and disabled states are treated as strong indicators of fragility because they directly indicate that the repository may no longer be normally maintained or available for development. Delayed updates and large issue counts are treated as moderate risk signals. Low popularity is included as a smaller risk factor because less visible projects may have a smaller user and maintainer base, although popularity alone does not guarantee quality or safety. Missing license information is also included because license clarity is an important factor in open source library selection and governance \parencite{liExploringFactorsMetrics2022,linakerHowCharacterizeHealth2022}.

\begin{equation}
\begin{split}
Fragility = & \,0.25 \times archived \\
& + 0.20 \times disabled \\
& + 0.20 \times norm(days\_since\_push) \\
& + 0.15 \times norm(days\_since\_update) \\
& + 0.10 \times norm(log(1 + open\_issues)) \\
& + 0.05 \times norm^{-1}(log(1 + stars)) \\
& + 0.05 \times missing\_license
\end{split}
\end{equation}

The resulting values are categorized into three levels for interpretability: low, medium, and high fragility. These categories are not intended to label a repository as unsafe by themselves. Instead, they provide a relative ranking that helps identify repositories and APKs that may require closer manual review. The fragility ranking for repositories detected in the APK corpus is shown in Figure~\ref{fig:github-fragility-score}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth,keepaspectratio]{image/top_repositories_by_fragility_score.png}
\caption{Top 10 repositories by fragility score in the APK corpus}
\label{fig:github-fragility-score}
\end{figure}

The distribution of fragility levels is shown in Figure~\ref{fig:github-fragility-distribution}. Most repositories fall into the low-fragility category (251 repositories), while a smaller group is classified as medium fragility (23 repositories). No repository in the analyzed corpus is classified as high fragility.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth,keepaspectratio]{image/repository_count_by_fragility_level.png}
\caption{Distribution of repositories across fragility levels in the APK corpus}
\label{fig:github-fragility-distribution}
\end{figure}

\subsection{Metric Relationships}
The three metrics capture different dimensions of repository characteristics and should be interpreted together. Figure~\ref{fig:popularity-vs-fragility-vs-activity} compares popularity and activity scores, with fragility level shown by color. The median popularity and activity lines divide the plot into four interpretive quadrants. The main observation is that activity appears to be a stronger indicator of fragility than popularity: above the median activity line, only a very small number of medium-fragility repositories appear. The low-activity and high-popularity quadrant is also almost empty, meaning that repositories with below-median activity are generally not among the most popular repositories in this corpus.

This pattern suggests that activity and lower fragility may partially reinforce each other. Active repositories are more likely to receive maintenance attention from other developers, which can reduce fragility over time. In contrast, stars and subscribers mainly indicate visibility or interest; no matter how popular a repository becomes, recent commits and updates appear to have a stronger relationship with fragility in this corpus.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth,keepaspectratio]{image/popularity_vs_activity_fragility.png}
\caption{Relationship between popularity, activity, and fragility level}
\label{fig:popularity-vs-fragility-vs-activity}
\end{figure}
...
At this point, the pipeline has identified the open-source libraries used in each analyzed APK. The validation question is whether there is any chance to see these libraries as a list inside the application itself. If an app provides an in-app open-source license page, then the pipeline findings can be qualitatively compared with that visible declaration. Therefore, a small-scale manual validation was conducted to check whether the detected library declarations are visible to end users inside Android applications.

\section{Sample Selection}
The validation sample consists of 27 applications selected using the following criteria:
\begin{itemize}
\item Applications in the corpus with more than 10 million installations
\item The application \textit{BudapestGO}, included due to frequent usage by author.
\item The top 5 APKs with the highest medium-fragility library usage intensity (as defined in later analysis)
\end{itemize}
The full list of apps in manual validation are provided in Appendix Table~\ref{tab:list-apps-manual-validation}.

\section{Manual Verification}
All selected applications were installed and tested using the Android Studio Emulator. Each application was explored manually, including settings menus, about pages, and other sections where license disclosures are typically located.

The inspection showed that almost none of the applications provide in-app declarations of open-source libraries. The only exception was \textit{BudapestGO}, where the in-app license information fully matched the libraries detected by the pipeline. A screenshot of the BudapestGO license screen is provided in Appendix Figure~\ref{fig:budapestgo-screenshot-licenses}.

\section{Accuracy Evaluation}
A quantitative accuracy evaluation is not meaningful in this context. Only one application in the sample provided visible license declarations.

\section{Limitations of Validation}
The validation is limited by the small and imbalanced sample size, because most selected applications come from high-installation or medium-fragility cases rather than from a category-balanced validation set. It is also limited by the lack of in-app license disclosures in most applications.
...
\subsection{Medium-Fragility Open Source Library Usage in APKs}
After the APK-level analysis, the repository-level fragility metrics were joined back to the APK/library evidence dataset. This makes it possible to identify which applications use libraries classified as having \textit{Medium} fragility and to measure how intensely those libraries appear inside each APK. In this analysis, usage intensity is measured through \texttt{classes\_matched}, which is used as a proxy for how strongly a detected library appears in the decoded APK codebase.

Across the largest analyzed corpus, the dataset contains \textbf{108,234} APK/library evidence rows. Of these, \textbf{106,368} rows could be joined to a GitHub repository with an assigned fragility level. Within this joined dataset, \textbf{739} rows correspond to libraries classified as \textit{Medium} fragility. These medium-fragility rows appear at least one time in \textbf{516} distinct APKs. This means that medium-fragility libraries are not limited to isolated cases; they appear across a meaningful subset of the analyzed application corpus size of \textbf{699}.

\subsubsection{How intense is Medium-fragility library use inside each APK?}
The intensity of medium-fragility usage is calculated as the share of class matches associated with medium-fragility libraries relative to all detected open-source class matches in the same APK:

\begin{equation}
MediumFragilityIntensity = \frac{medium\_fragility\_classes\_matched}{apk\_total\_classes\_matched} \times 100
\end{equation}

This measure does not claim to represent runtime execution frequency. Instead, it captures how much of the detected open-source code evidence inside an APK is associated with medium-fragility repositories. Applications with high values should therefore be interpreted as having stronger structural exposure to libraries with medium fragility.

\subsubsection{Which APKs use open source libraries with Medium fragility?}
The first question is which APKs contain at least one open-source library classified as \textit{Medium} fragility. The analysis identifies \textbf{516} such APKs. For each APK, the aggregation records the number of medium-fragility evidence rows, the number of unique medium-fragility repositories, the number of matched classes associated with these libraries, and the percentage of the APK's total detected open-source class matches represented by medium-fragility libraries.

The ranking is based on intensity of fragile library use (\texttt{medium\_fragility\_intensity\_pct\_of\_apk}). The highest observed values include \textit{Cabrillo Credit Union} at \textbf{10.67\%}, \textit{UK Credit Union} at \textbf{9.35\%}, and \textit{BrightStar CU Mobile} at \textbf{6.22\%} (Figure~\ref{fig:top-apks-medium-fragility-intensity}). These applications therefore show the strongest relative presence of medium-fragility libraries within their detected open-source class usage.



\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth,keepaspectratio]{image/top_apps_by_medium_fragility_intensity.png}
    \caption{Top 25 APKs by medium-fragility library usage intensity}
    \label{fig:top-apks-medium-fragility-intensity}
    \end{figure}

\subsubsection{Which APKs use Medium-fragility libraries but uncited (cited = 0)?}
The second part of the analysis focuses only on medium-fragility rows where \texttt{cited = 0}. These are cases where the pipeline found code evidence of a medium-fragility library, but no corresponding license declaration was identified in the APK evidence. The dataset contains \textbf{346} uncited medium-fragility rows, appearing across \textbf{268} distinct APKs.

The leading APKs by uncited medium-fragility intensity include \textit{in.kontakt}, \textit{Tickz. App broker for trading}, and \textit{Los de la Isla CrossFit} (Figure~\ref{fig:top-apks-uncited-medium-fragility-intensity}). These cases are important because the relevant medium-fragility libraries are detected through code evidence but are not visible through license citation. As a result, even a technically skilled user would have little practical opportunity to identify these libraries from the APK disclosure information alone and react if one of them became risky.

In Figure~\ref{fig:top-apks-uncited-medium-fragility-intensity}, FINANCE is the most frequent category with 12 applications. This partly reflects the overall corpus composition, where FINANCE is also the largest category. However, the presence of many finance apps in the uncited medium-fragility group also indicates that financial-sector applications need particularly careful dependency visibility and license-disclosure practices.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth,keepaspectratio]{image/top_apps_by_uncited_medium_fragility_intensity.png}
\caption{Top 25 APKs by uncited medium-fragility library usage intensity}
\label{fig:top-apks-uncited-medium-fragility-intensity}
\end{figure}
...
\item Repository-level analysis shows that most detected GitHub libraries have low fragility, but medium-fragility libraries still appear in a meaningful subset of APKs (516 out of 699 APKs).
\item Uncited medium-fragility libraries are especially important because users and auditors cannot easily identify them from license disclosure alone.
\item Finance applications appear frequently among the top APKs with uncited medium-fragility intensity, indicating the need for stronger dependency visibility in sensitive sectors.
\end{itemize}

Overall, the results show that the main issue is not only the presence of open-source libraries, but the gap between actual library usage and what is disclosed to users or reviewers.
...
The final corpus contains 636 unique repository URLs and an average of 154.84 detected library occurrences per APK. The libraries are mainly detected through Java and Kotlin package fingerprints. Repository-level analysis also showed that most detected GitHub repositories fall into the low-fragility group, while 23 repositories were classified as medium fragility and none as high fragility.

\textbf{RQ4: Which Android applications show the highest dependency on fragile open source libraries?}
The analysis identified 346 uncited medium-fragility rows across 268 APKs. The leading applications by uncited medium-fragility intensity include \textit{in.kontakt}, \textit{Tickz. App broker for trading}, and \textit{Los de la Isla CrossFit}. Among the top 25 APKs in this ranking, FINANCE is the most frequent category, indicating that dependency visibility is especially important for sensitive financial applications.

Overall, the practical part of the thesis was applicable and completed: the APK collection was built, the decoding and matching pipeline was implemented, GitHub metadata was collected, repository metrics were calculated, and the final dataset was analyzed. The pipeline can also target a standalone APK, add it to the corpus, identify its open-source library ingredients, assess which ones appear fragile, and compare it with other APKs. Some expectations were only partially achieved. The corpus size remained limited by hardware and lookup time, the sample was not category-balanced, manual validation was limited because most applications did not expose in-app license information, and the method cannot prove runtime execution of a library. Therefore, the results should be interpreted as evidence of embedded library presence and disclosure gaps, not as a complete runtime security assessment.
...
\begin{table}[htbp]
    ...
    Medium fragility library occurrences & 2 \\
    Uncited medium fragility library occurrences & 1 \\
    Total intensity of medium fragility libraries & 0.10\% \\
    \hline
    \end{tabular}
    
    {\footnotesize\textit{Note.} Two medium-fragility libraries were detected: \url{https://github.com/chrisbanes/snapper/}, which was cited, and \url{https://github.com/mapbox/mapbox-gl-native}, which was not cited.}
    \end{table}
...
GitHub repositories found & 257 \\
GitHub repositories not found or returned error & 17 \\
\hline
\end{tabular}
